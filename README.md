...
- التطبيق يستخدم SQLite محليًا بدون Firebase أو Backend.
- قاعدة بيانات الإنتاج اسمها `almajed_pro.db`.
- عند التثبيت الأول تبدأ قاعدة البيانات فارغة من المنتجات والتصنيفات والعملاء والموردين.
- يتم إنشاء مستخدم النظام الافتراضي فقط: اسم المستخدم `000` وكلمة المرور `000`.
- قسم المشتريات لا ينشئ منتجات تلقائيًا؛ يجب إنشاء المنتج أولًا من قسم المنتجات.
- أرقام القيود المحاسبية مرتبطة بمصدر العملية (`sourceType/sourceId`) لمنع تصادم UNIQUE عند الحفظ المتزامن أو إعادة البناء.
