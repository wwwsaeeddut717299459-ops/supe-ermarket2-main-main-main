...
class PurchaseService {
  ...
  Future<int> createPurchase({
    ...
    int createdPurchaseId = 0;

    await db.transaction(() async {
      final normalizedInvoice = await db.nextInvoiceNumber();
      ...
      for (final item in items) {
        ...
        if (item.sellingPrice < 0) throw Exception('سعر البيع غير صحيح');
        if (item.discount < 0 ||
      ...
      for (final item in items) {
        ...
        // الشراء لا ينشئ منتجات تلقائيًا. المنتج يجب أن يكون مسجلاً
        // مسبقًا من قسم المنتجات حتى تبقى قاعدة البيانات تحت سيطرة المستخدم.
        if (product == null) {
          throw Exception(
            'المنتج "${item.productName.trim()}" غير موجود. ' 
            'أضفه أولًا من قسم المنتجات ثم أعد محاولة الشراء.',
          );
        }
    ...
    // التحديث المحاسبي خارج Transaction لضمان استقرار القيود بدون تكرار
    try {
  ...
  Future<void> cancelPurchase(int purchaseId) async {
    await db.transaction(() async {
      ...
      await db.purchasesDao.updatePurchase(
        purchase.toCompanion(true).copyWith(
    ...
    // إعادة بناء القيود بعد إلغاء الشراء خارج المعاملة
    try {
      await AccountingService(db).rebuild();
    } catch (_) {}
  }
