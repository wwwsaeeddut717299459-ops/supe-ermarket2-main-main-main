...
class SaleService {
  ...
  Future<int> createSale({
    ...
    await db.transaction(() async {
      // الرقم يُحجز داخل نفس المعاملة لضمان عدم التكرار.
      final generatedInvoiceNumber = await db.nextInvoiceNumber();
      ...
      for (final item in items) {
        ...
        final itemTotal = (unitPrice * quantity) - itemDiscount;
      ...
      for (final item in items) {
        ...
        // --------------------------------------------------------
        // حساب الرصيد الجديد للمخزون وتحديثه
        // --------------------------------------------------------
        ...
        await db.stockMovementsDao.insertMovement(
          StockMovementsCompanion.insert(
            ...
            referenceNumber: Value(generatedInvoiceNumber),
            referenceId: Value(saleId),
            notes: Value(
              'بيع من الفاتورة $generatedInvoiceNumber',
            ),
          ),
        );
      }
      ...
      if (paymentMethod == 'credit' && customerId != null && remaining > 0) {
        await db.customerTransactionsDao.insertTransaction(
          CustomerTransactionsCompanion.insert(
            ...
            notes: Value(
              'دين ناتج عن الفاتورة $generatedInvoiceNumber',
            ),
          ),
        );
      }
    ...
    // ==========================================================
    // 6. التحديث المحاسبي (خارج المعاملة الرئيسية لتفادي تكرار القيود)
    // ==========================================================
    try {
      await AccountingService(db).rebuild();
    } catch (_) {
      // إهمال التأخير لتفادي إيقاف عملية إتمام الفاتورة الناجحة
    }
