...
class _SalesPageState extends ConsumerState<SalesPage> {
  ...
  bool _handleGlobalKeyInput(KeyEvent event) {
    if (event is KeyDownEvent) {
      ...
      // إذا كان التركيز بالفعل على حقل الباركود، دع الحقل يتعامل مع الإدخال بنفسه لتجنب التكرار
      if (primaryFocus == _barcodeFocusNode) {
      ...
      if (event.logicalKey == LogicalKeyboardKey.enter) {
        if (_barcodeBuffer.isNotEmpty) {
          final barcode = _barcodeBuffer.toString().replaceAll('\r', '').replaceAll('\n', '').trim();
          _barcodeBuffer.clear();
  ...
  Future<void> _addByBarcode() async {
    final barcode = _barcodeController.text.replaceAll('\r', '').replaceAll('\n', '').trim();
    _barcodeBuffer.clear(); // تفريغ المؤقت لضمان عدم وجود متبقيات
