...
class _InvoicesPageState extends ConsumerState<InvoicesPage> {
  ...
  Future<void> _printSale(InvoiceSummaryRow row) async {
    if (row.type != 'sale') return;

    try {
      final salesRepository = ref.read(salesRepositoryProvider);
      final settingsRepository = ref.read(invoiceSettingsRepositoryProvider);
      final sale = await salesRepository.getById(row.id);
      if (sale == null) {
        throw Exception('لم يتم العثور على الفاتورة.');
      }
      final items = await salesRepository.getItems(row.id);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم إرسال الفاتورة إلى الطابعة بنجاح.')),
        );
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'تعذرت الطباعة.\nتحقق من اتصال الطابعة وإعداداتها ثم حاول إعادة الطباعة.\n$e',
          ),
