...
class BackupService {
  ...
  Future<File> databaseFile() async {
    ...
    return File(p.join(directory.path, 'almajed_pro.db'));
  }
  ...
  Future<List<File>> _supportFiles() async {
    ...
    await for (final entity in support.list(
      ...
      // لا تدخل قاعدة البيانات القديمة التي كانت تستخدمها الإصدارات
      // السابقة في النسخة الاحتياطية الجديدة.
      final basename = p.basename(normalized);
      if (basename == 'almajed_pro_v12.db' ||
          basename == 'almajed_pro_v12.db-wal' ||
          basename == 'almajed_pro_v12.db-shm') {
        continue;
      }
      files.add(entity);
    }
  ...
  Future<File> createBackup({String? destinationDirectory}) async {
    ...
    try {
      final currencyFile = await CurrencyService().settingsFile();
      if (await currencyFile.exists()) {
        currencySettings = jsonDecode(await currencyFile.readAsString())
  ...
  /// يستبدل ملفًا على Windows مع إعادة المحاولة لأن SQLite/Drift قد يحرر
  /// الـ memory mapping بعد إغلاق الاتصال مباشرةً بزمن قصير جدًا.
  Future<void> _copyWithRetry(
    ...
    throw Exception('تعذر استبدال ملف قاعدة البيانات بعد إغلاقه: $lastError');
  }
  ...
  Future<void> _writeWithRetry(
    ...
    throw Exception('تعذر كتابة ملفات الاستعادة بعد إغلاق قاعدة البيانات: $lastError');
  }
  ...
  Future<void> restoreFrom(String sourcePath) async {
    ...
    if (extension == '.db') {
      ...
      // يجب إغلاق اتصال SQLite قبل لمس الملف على Windows.
      await db.close();
      await _copyWithRetry(source, target);
      return;
    }
    if (extension != '.zip')
    ...
    await db.close();
    String restoreRelativePath(String relative) {
      // توافق مع النسخ الاحتياطية التي أنشأتها الإصدارات السابقة.
      if (relative == 'almajed_pro_v12.db') return 'almajed_pro.db';
      if (relative == 'almajed_pro_v12.db-wal') return 'almajed_pro.db-wal';
      if (relative == 'almajed_pro_v12.db-shm') return 'almajed_pro.db-shm';
      return relative;
    }

    final expectedPaths = entries
        .map((entry) => restoreRelativePath(
    ...
    for (final entry in entries) {
      final item = entry as Map<String, dynamic>;
      final sourceRelative = item['path'] as String;
      if (p.isAbsolute(sourceRelative) ||
          p.normalize(sourceRelative).startsWith('..')) {
        throw Exception('مسار غير آمن داخل النسخة');
      }
      final relative = restoreRelativePath(sourceRelative);
      final target = File(p.join(support.path, relative));
      final archiveFile = archive.findFile(sourceRelative)!;
