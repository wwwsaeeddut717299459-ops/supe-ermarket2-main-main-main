...
class AccountingService {
  ...
  Future<void> _post({
    ...
    // رقم القيد يجب أن يكون حتميًا وفريدًا حتى عند تنفيذ عدة عمليات
    // حفظ/بيع/تعديل بسرعة أو أثناء إعادة بناء القيود. الاعتماد على
    // microsecondsSinceEpoch كان يسمح بتصادم نادر وينتج UNIQUE constraint failed.
    final safeSourceType = sourceType
        .trim()
        .replaceAll(RegExp(r'[^A-Za-z0-9_-]+'), '_');
    final entryNumber = 'JE-$safeSourceType-$sourceId';
  ...
  Future<void> postManual({
    ...
    await db.transaction(() async {
      await ensureChart();
      // معرف العملية اليدوية لا يعتمد على دقة الساعة وحدها.
      // استخدام أكبر قيمة id موجودة + 1 يمنع تخطي قيد يدوي بسبب تصادم زمني.
      final latestManual = await db.customSelect(
        'SELECT MAX(source_id) AS max_id FROM journal_entries WHERE source_type = ?',
        variables: [Variable.withString('manual')],
        readsFrom: {db.journalEntries},
      ).getSingle();
      final maxManualId = (latestManual.data['max_id'] as num?)?.toInt() ?? 0;
      final sourceId = maxManualId + 1;
      await _post(
        sourceType: 'manual',
        sourceId: sourceId,
        description: clean,
